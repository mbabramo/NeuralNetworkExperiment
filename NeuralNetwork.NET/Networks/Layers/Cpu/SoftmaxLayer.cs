...
{
    /// <summary>
    /// An output layer with the softmax activation function and log-likelihood cost function
    /// </summary>
    internal class SoftmaxLayer : OutputLayerBase
    {
        ...
        public SoftmaxLayer(in TensorInfo input, int outputs, WeightsInitializationMode weightsMode, BiasInitializationMode biasMode)
            : base(input, outputs, ActivationType.Softmax, CostFunctionType.LogLikelihood, weightsMode, biasMode) { }

        public SoftmaxLayer(in TensorInfo input, int outputs, [NotNull] float[] weights, [NotNull] float[] biases)
            : base(input, outputs, weights, biases, ActivationType.Softmax, CostFunctionType.LogLikelihood) { }
        ...
        public new static INetworkLayer Deserialize([NotNull] Stream stream)
        {
            ...
            if (!stream.TryRead(out CostFunctionType cost) && cost == CostFunctionType.LogLikelihood) return null;
